...
void ConsoleCommandHandler::ParseCommand(std::string command, IRCClient* client)
{
    if (_commands.empty())
    {
        std::cout <<"\x1b[32;1m"<< "No commands available."<<"\x1b[0m" << std::endl;
        return;
    }
    ...
    {
        std::cout <<"\x1b[32;1m"<< "No such command" <<"\x1b[0m" << std::endl;
        return;
    }

    if (++argCount < itr->second.argCount)
    {
        std::cout <<"\x1b[32;1m"<< "Insuficient arguments"<<"\x1b[0m" << std::endl;
        return;
    }
